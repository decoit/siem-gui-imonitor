...
public class TicketServiceImpl implements TicketService {
	...
	private Ticket createTicketForIncident(Incident inc) throws ExternalServiceException {
		...
		// If explanations are present, add them to the initial text
		if(!inc.getExplanations().isEmpty()) {
			StringBuilder sbExpl = new StringBuilder("[b]Erklärungen:[/b]");
			sbExpl.append("[bq]");

			for(IncidentExplanation iExpl : inc.getExplanations()) {
				sbExpl.append("[p]");
				sbExpl.append(iExpl.getExplanation());
				sbExpl.append("[/p]");
			}

			sbExpl.append("[/bq]");

			sbText.append(sbExpl);
		}

		// If recommendations are present, add them to the initial text
		if(!inc.getRecommendations().isEmpty()) {
			StringBuilder sbRec = new StringBuilder("[b]Handlungsempfehlungen:[/b]");
			sbRec.append("[bq]");
