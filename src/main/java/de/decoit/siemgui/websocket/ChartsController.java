...
public class ChartsController {
	...
	/**
	 * This mapping responds with the chart of today's events per hour.
	 * It is mapped to /charts/eventstoday and the response is sent to /queue/charts (user specific)
	 *
	 ...
	 */
	@MessageMapping("/charts/eventstoday")
	@SendToUser("/queue/charts")
	@Secured("ROLE_SIEM_USER")
	public MsgChart requestChartEventsToday(MsgChartRequest reqMsg, Principal principal) throws ExternalServiceException {
		if(LOG.isDebugEnabled()) {
			LOG.debug("requestChartEventsToday, incoming message: " + reqMsg.toString());
		}

		MsgChart msg = chartsServ.getEventsTodayChart(reqMsg.getChartIndex());

		if(LOG.isDebugEnabled()) {
			LOG.debug("requestChartEventsToday, outgoing message: " + msg.toString());
		}
	...
	/**
	 * This mapping responds with the chart of today's events per sensor.
	 * It is mapped to /charts/eventstodaypersensor and the response is sent to /queue/charts (user specific)
	 *
	 * @param reqMsg    Inbound message containing the chart index
	 * @param principal Authentication information provided by Spring Security
	 * @return STOMP message containing the chart information
	 ...
	 * @throws ExternalServiceException if contacting external services failed
	 */
	@MessageMapping("/charts/eventstodaypersensor")
	@SendToUser("/queue/charts")
	@Secured("ROLE_SIEM_USER")
	public MsgChart requestChartEventsTodayPerSensor(MsgChartRequest reqMsg, Principal principal) throws ExternalServiceException {
		if(LOG.isDebugEnabled()) {
			LOG.debug("requestChartEventsTodayPerSensor, incoming message: " + reqMsg.toString());
		}

		MsgChart msg = chartsServ.getEventsTodayPerSensorChart(reqMsg.getChartIndex());

		if(LOG.isDebugEnabled()) {
			LOG.debug("requestChartEventsTodayPerSensor, outgoing message: " + msg.toString());
		}
