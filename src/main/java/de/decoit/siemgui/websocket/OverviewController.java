...
import de.decoit.siemgui.domain.tickets.Ticket;
import de.decoit.siemgui.domain.tickets.TicketStatus;
...
import de.decoit.siemgui.stomp.msgs.incoming.MsgOverviewEventsRequest;
import de.decoit.siemgui.stomp.msgs.outgoing.MsgErrorNotification;
import de.decoit.siemgui.stomp.msgs.outgoing.MsgOverviewEvents;
...
public class OverviewController {
	...
	/**
	 * This mapping responds with the overview summary of event information.
	 * It maps to /overview/reqinfo/events and responds to /queue/overview/reqinfo/events (user specific)
	 *
	 * @param inMsg Incoming request message
	 * @param principal Authentication information provided by Spring Security
	 * @return STOMP message containing the summary
	 *
	 * @throws ExternalServiceException if contacting external services failed
	 */
	@MessageMapping("/overview/reqinfo/events")
	@SendToUser("/queue/overview/reqinfo/events")
	@Secured("ROLE_SIEM_USER")
	public MsgOverviewEvents requestOverviewEvents(MsgOverviewEventsRequest inMsg, Principal principal) throws ExternalServiceException {
		if(LOG.isDebugEnabled()) {
			LOG.debug("requestOverviewEvents, incoming message: " + inMsg.toString());
		}

		EventRequestTimeframe timeframe = inMsg.getTimeframe();
		long count;

		switch(timeframe) {
			case TODAY:
				count = eventServ.countEventsToday();
				break;
			case YESTERDAY:
				count = eventServ.countEventsYesterday();
				break;
			case THIS_WEEK:
				count = eventServ.countEventsThisWeek();
				break;
			case LAST_WEEK:
				count = eventServ.countEventsLastWeek();
				break;
			default:
				throw new IllegalArgumentException("Unknown event request timeframe: " + timeframe);
		}

		MsgOverviewEvents msg = new MsgOverviewEvents();

		msg.setTimeframe(timeframe);
		msg.setCount(count);

		if (LOG.isDebugEnabled()) {
			LOG.debug("requestOverviewEvents, outgoing message: " + msg.toString());
		}
