...
angular.module('SiemGui')
	.factory('WebSocketService', ['$log', '$rootScope', function($log, $rootScope) {
		 ...
		 * @param {Boolean} [debug] Enable debugging of SockJS and STOMP, defaults to false
		 * @returns {undefined}
		 */
		Service.connect = function(wsEndpoint, callback, headers, debug) {
			if(!isConnected) {
				var custHeaders = {};
				...
				if(enableDebug) {
					sockJsOptions.debug = true;
				}

				ws = new SockJS(wsEndpoint, sockJsOptions);
